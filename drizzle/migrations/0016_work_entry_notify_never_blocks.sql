-- trg_work_entry_notify_worker: actor name from profiles.display_name by
-- profiles.user_id (profiles has no full_name), and the whole notification
-- part inside an exception block so a notification can never abort a work
-- entry insert. Starting point: live definition (pg_get_functiondef).
    ...
    insert into public.notifications (
      ...
      jsonb_build_object(
        ...
        'message_vars', jsonb_build_object(
          ...
          'actor', coalesce(v_actor_name, 'Vlasnik')
        )
      ),
      false,
      'active'
...
REVOKE ALL ON FUNCTION public.trg_work_entry_notify_worker() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.trg_work_entry_notify_worker() FROM anon;
