CREATE OR REPLACE FUNCTION public.move_project_to_business_profile(
  p_project_id uuid,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = p_business_profile_id AND user_id = v_uid
  ) THEN
    RAISE EXCEPTION 'business profile not found' USING ERRCODE = '42501';
  END IF;

  IF v_current IS NOT NULL THEN
    RAISE EXCEPTION 'project already belongs to a business profile' USING ERRCODE = '42501';
  END IF;

  UPDATE public.projects
  SET business_profile_id = p_business_profile_id,
      updated_at = now()
  ...
  WHERE project_id = p_project_id
    AND user_id = v_uid
    AND business_profile_id IS DISTINCT FROM p_business_profile_id;
  GET DIAGNOSTICS v_expenses = ROW_COUNT;

  UPDATE public.incoming_invoices ii
  SET business_profile_id = p_business_profile_id
  WHERE ii.user_id = v_uid
    AND ii.business_profile_id IS DISTINCT FROM p_business_profile_id
    AND ii.paid_expense_id IN (
      SELECT e.id FROM public.expenses e WHERE e.project_id = p_project_id AND e.user_id = v_uid
    );
  GET DIAGNOSTICS v_invoices = ROW_COUNT;

  -- Engagements keep person identity: reuse an existing person with the same
  -- name inside the target company, otherwise create a NEW person there.
  ...
  LOOP
    ...
    IF v_target_worker IS NULL THEN
      ...
      VALUES (v_uid, p_business_profile_id, r.first_name, r.last_name, r.phone, r.note, r.linked_user_id)
...
REVOKE ALL ON FUNCTION public.move_project_to_business_profile(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.move_project_to_business_profile(uuid, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.move_project_to_business_profile(uuid, uuid) TO authenticated;