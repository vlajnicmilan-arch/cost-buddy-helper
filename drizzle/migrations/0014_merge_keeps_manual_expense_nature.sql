...
BEGIN
  ...
  END IF;
  -- Prazna/NULL valuta = osnovna valuta EUR (ručni retci često nemaju upisanu valutu)
  IF UPPER(COALESCE(v_manual.currency,'EUR')) IS DISTINCT FROM UPPER(COALESCE(v_bank.currency,'EUR')) THEN
  ...
  UPDATE public.expenses
     SET bank_transaction_id  = v_bank.bank_transaction_id,
         ...
         -- Spojeni redak zadržava vrstu ručnog retka (npr. krug_settlement).
         expense_nature       = v_manual.expense_nature,
