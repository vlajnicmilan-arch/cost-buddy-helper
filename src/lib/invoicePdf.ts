// PDF generator za račune (INTERNI tracker — NIJE službeni porezni dokument).
// Reuse: pdfBranding, pdfFooter, fileExport.
...
export interface GenerateInvoicePdfOpts {
  mode?: ExportMode;
...
export async function generateInvoicePdf(
  ...
  const business = await fetchBusinessHeader(invoice.business_profile_id);
  ...
  doc.setFontSize(18);
  doc.text(t('invoices.pdf.title', 'PREGLED RAČUNA'), margin, 12);
  doc.setFont('Inter', 'normal');
  doc.setFontSize(11);
  doc.text(invoice.invoice_number, margin, 20);

  const rightX = pageWidth - margin;
  doc.setFontSize(9);
  doc.text(`${t('invoices.pdf.issueDate', 'Datum izdavanja')}: ${formatDate(invoice.issue_date)}`, rightX, 12, { align: 'right' });
  if (invoice.due_date) {
    doc.text(`${t('invoices.pdf.dueDate', 'Dospijeće')}: ${formatDate(invoice.due_date)}`, rightX, 18, { align: 'right' });
  }
  ...
  doc.setFontSize(8);
  doc.text(t('invoices.pdf.issuer', 'IZDAVATELJ'), margin, y);
  doc.text(t('invoices.pdf.client', 'PRIMATELJ'), pageWidth / 2 + 4, y);
  y += 4;
  doc.setTextColor(...BRAND_DARK);
  doc.setFont('Inter', 'bold');
  doc.setFontSize(10);
  doc.text(business?.company_name || '—', margin, y);
  doc.text(invoice.client_name, pageWidth / 2 + 4, y);
  y += 5;
  doc.setFont('Inter', 'normal');
  doc.setFontSize(9);
  ...
  if (invoice.client_oib) clientLines.push(`OIB: ${invoice.client_oib}`);
  ...
  const head = [[
    '#',
    t('invoices.pdf.itemDescription', 'Opis'),
    t('invoices.pdf.qty', 'Kol.'),
    t('invoices.pdf.unit', 'Jed.'),
    t('invoices.pdf.unitPrice', 'Cijena'),
    t('invoices.pdf.vatPct', 'PDV%'),
    t('invoices.pdf.lineTotal', 'Iznos'),
  ]];
  ...
  doc.setFontSize(9);
  doc.setTextColor(...BRAND_DARK);
  doc.text(`${t('invoices.subtotal', 'Osnovica')}:`, totalsX, y);
  doc.text(fmt(Number(invoice.subtotal) || 0, currencyCode), valX, y, { align: 'right' });
  y += 5;
  doc.text(`${t('invoices.vat', 'PDV')}:`, totalsX, y);
  doc.text(fmt(Number(invoice.vat_amount) || 0, currencyCode), valX, y, { align: 'right' });
  y += 6;
  ...
  doc.setFont('Inter', 'bold');
  doc.setFontSize(11);
  doc.text(`${t('invoices.total', 'Ukupno')}:`, totalsX, y + 1.5);
  doc.text(fmt(Number(invoice.total_amount) || 0, currencyCode), valX, y + 1.5, { align: 'right' });
  y += 12;

  // ===== Payment status =====
  if (typeof opts.paid === 'number') {
    const paid = opts.paid;
    const remaining = (Number(invoice.total_amount) || 0) - paid;
    doc.setTextColor(...BRAND_DARK);
    doc.setFont('Inter', 'normal');
    doc.setFontSize(9);
    doc.text(`${t('invoices.paid', 'Plaćeno')}: ${fmt(paid, currencyCode)}`, margin, y);
    y += 5;
    doc.text(`${t('invoices.remaining', 'Preostalo')}: ${fmt(Math.max(0, remaining), currencyCode)}`, margin, y);
    y += 6;
  }
  ...
  if (invoice.notes) {
    ...
    doc.setFont('Inter', 'normal');
    doc.setFontSize(9);
    doc.text(t('invoices.notes', 'Napomena') + ':', margin, y);
    y += 4;
    doc.setTextColor(...BRAND_DARK);
  ...
  if (business && business.is_vat_payer === false) {
    doc.setTextColor(...BRAND_MUTED);
    doc.setFontSize(8);
    doc.text(t('invoices.pdf.nonVatNote', 'Izdavatelj nije obveznik PDV-a.'), margin, y);
    y += 5;
  }
  ...
  doc.setFontSize(8);
  const disclaimer = t(
    'invoices.pdf.disclaimer',
    'Interni pregled računa za vlastitu evidenciju i komunikaciju s klijentom. Nije fiskalizirani porezni račun niti službeni dokument u smislu poreznih i računovodstvenih propisa.'
  );
