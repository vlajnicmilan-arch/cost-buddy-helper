/**
 * CARD MATCH — broj kartice kao PRIMARNI signal o tome tko je platio.
 *
 * Bankovni redak („Revolut**5385* Dublin", „462765XXXXXX2081",
 * „Kartica: …1542") često nosi masku kartice. Ako se zadnje 4 znamenke
 * poklope s korisnikovom upisanom karticom (`payment_source_cards`), platilac
 * je determinističan — bez pogađanja po nazivu trgovca.
 *
 * Pravila koja se ne pregovaraju:
 *  - Više različitih pogodaka → `null` (radije ništa nego kriv novčanik).
 *  - Nijedan pogodak → `null`; naziv („Revolut", „Aircash") je samo rezerva
 *    na višoj razini, ne ovdje.
 *  - Brojevi koje korisnik nije upisao se ignoriraju.
 *
 * Zrcalo za edge funkcije: `supabase/functions/_shared/cardMatch.ts`.
 * Jezgra između SHARED CORE markera mora biti identična u obje datoteke —
 * čuva ju `cardMatchMirror.test.ts`.
 */
