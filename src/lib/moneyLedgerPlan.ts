/**
 * JEZGRA ODLUKE O RETKU NOVCA — jedan rječnik za uvoz i (uskoro) sinkronizaciju.
 *
 * Danas na isto pitanje — „što je ovaj redak novca" — odgovaraju dva mjesta:
 * `planExecution` (uvoz) i `decideBankSyncRow` (sinkronizacija). Ovaj modul je
 * prvi korak prema jednoj odluci: zatvoren popis ishoda, razlog i, gdje postoji,
 * odabrani kandidat. Bez mreže, bez baze, bez pisanja.
 *
 * Pravila koja se ne pregovaraju:
 *  - `userId` je OBAVEZAN. Bez njega jezgra baca grešku.
 *  - Kandidat koji nije istog vlasnika NE POSTOJI za ovaj redak — jezgra ga
 *    izbacuje prije svake odluke i nikad ga ne smije vratiti.
 *  - Jezgra ne piše i ne određuje KAKO se upisuje; samo ŠTO je redak.
 *
 * Zrcalo za edge funkcije: `supabase/functions/_shared/moneyLedgerPlan.ts`.
 * Jezgra između SHARED CORE markera mora biti identična u obje datoteke —
 * čuva ju `moneyLedgerPlanMirror.test.ts`.
 */
