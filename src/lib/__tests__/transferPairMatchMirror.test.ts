/**
 * Zrcalo `src/lib/transferPairMatch.ts` ↔
 * `supabase/functions/_shared/transferPairMatch.ts`. Pada čim se raziđu.
 */
...
describe('transferPairMatch mirror', () => {
  it('jezgra klijenta i edge kopije je identična', () => {
    expect(core('supabase/functions/_shared/transferPairMatch.ts'))
      .toBe(core('src/lib/transferPairMatch.ts'));
  });
});
