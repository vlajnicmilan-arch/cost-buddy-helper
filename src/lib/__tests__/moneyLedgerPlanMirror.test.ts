/**
 * Zrcalo `src/lib/moneyLedgerPlan.ts` ↔
 * `supabase/functions/_shared/moneyLedgerPlan.ts`. Pada čim se raziđu.
 */
...
describe('moneyLedgerPlan mirror', () => {
  it('jezgra klijenta i edge kopije je identična', () => {
    expect(core('supabase/functions/_shared/moneyLedgerPlan.ts'))
      .toBe(core('src/lib/moneyLedgerPlan.ts'));
  });
});
