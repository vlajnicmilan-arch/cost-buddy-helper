 ...
 * 
 * Usage: useBackButton(isOpen, onClose, priority?, label?)
 * - priority: higher number = closed first when multiple dialogs open (default: 0)
 * - label: optional diagnostic label for stack telemetry
 */

let idCounter = 0;

export function useBackButton(isOpen: boolean, onClose: () => void, priority = 0, label?: string) {
