...
/**
 * Dashboard blok: NEPLAĆENI ULAZNI RAČUNI (obveze prema dobavljačima).
 *
 * Parnjak `UnpaidInvoicesWidget` (što meni duguju) — ovo je što ja dugujem.
 * TVRDO PRAVILO: iznos je obveza, ne trošak. Ne ulazi ni u jedan zbroj na
 * dashboardu; trošak nastaje tek kroz „Plaćeno".
 *
 * Blok se ne renderira kad nema neplaćenih.
 */
export const IncomingInvoicesWidget = ({ variant = 'default' }: IncomingInvoicesWidgetProps = {}) => {
  ...
  const summary = useMemo(() => summarizeIncomingInvoices(invoices), [invoices]);
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <FileInput className={`w-4 h-4 shrink-0 ${hasOverdue ? 'text-destructive' : 'text-muted-foreground'}`} />
            <div className="min-w-0">
              <p className={monarch
                ? 'text-[10px] font-medium uppercase tracking-widest text-muted-foreground'
                : 'text-[10px] text-muted-foreground'}>
                {t('eracun.widget.title', 'Ulazni računi — neplaćeno')}
              </p>
              <p className={monarch
