/**
 * MONEY DIRECTION — Deno zrcalo modula `src/lib/moneyDirection.ts`.
 *
 * NE mijenjaj ovu datoteku ručno bez iste izmjene u `src/lib/moneyDirection.ts`:
 * jezgra između SHARED CORE markera mora biti bajt-identična, što provjerava
 * `src/lib/__tests__/moneyDirectionMirror.test.ts`.
 *
 * Edge funkcije ne mogu importati iz `src/`, pa je zrcalo jedini način da
 * bankovna sinkronizacija odlučuje o smjeru istim kodom kao uvoz izvoda.
 */
