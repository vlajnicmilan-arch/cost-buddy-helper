 ...
 */
/**
 * Ubrizgavanje XML parsera. U pregledniku i testovima koristi se globalni
 * `DOMParser`; u Deno runtimeu (edge funkcije) nema globalnog DOM-a, pa
 * pozivatelj postavlja parser prije prve upotrebe. Implementacija ostaje
 * JEDNA — bez duplikata po okruženju.
 */
type XmlParserLike = { parseFromString: (xml: string, type: string) => Document };
let injectedParser: XmlParserLike | null = null;

export const setUblXmlParser = (parser: XmlParserLike | null): void => {
  injectedParser = parser;
};

const resolveParser = (): XmlParserLike => {
  if (injectedParser) return injectedParser;
  const g = globalThis as unknown as { DOMParser?: new () => XmlParserLike };
  if (!g.DOMParser) throw new EracunParseError('not_xml');
...
export const parseUbl = (xml: string): EracunInvoice => {
  ...
  const doc = resolveParser().parseFromString(xml, 'application/xml');
