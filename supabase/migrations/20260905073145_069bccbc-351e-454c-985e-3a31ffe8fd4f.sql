ALTER TABLE public.incoming_invoices ALTER COLUMN invoice_number DROP NOT NULL;
...
BEGIN
  ...
  -- ZAPIS U KNJIGE: svaka greška baze se hvata, bilježi i prevodi u ok:false.
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    PERFORM public.mail_confirm_log_reject(
      v_uid, p_item_id, 'baza:' || v_sqlstate || ' ' || COALESCE(v_sqlerrm, ''),
      NULL, v_allow_no_oib, v_allow_no_number);
    RETURN jsonb_build_object('ok', false, 'reason', 'baza', 'db_code', v_sqlstate);
  END;
