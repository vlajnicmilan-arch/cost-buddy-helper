-- Faza 7: Automatsko stvaranje faze projekta iz odobrene odluke

-- 1) Nove kolone na project_milestones
...
-- 2) Idempotencija — jedna odluka može stvoriti max jednu (aktivnu) fazu
CREATE UNIQUE INDEX IF NOT EXISTS uniq_project_milestone_source_decision
  ON public.project_milestones(source_decision_id)
  WHERE source_decision_id IS NOT NULL AND deleted_at IS NULL;

-- 3) Prošireni after-trigger: uz aneks stvara i milestone (atomarno, idempotentno)
...
BEGIN
  ...
  IF NEW.action = 'accept' AND v_decision.contract_amendment_id IS NULL THEN
    ...
    IF v_effective_price IS NOT NULL AND v_effective_price <> 0 THEN
      ...
      -- Idempotencija: unique index na source_decision_id + ON CONFLICT DO NOTHING.
