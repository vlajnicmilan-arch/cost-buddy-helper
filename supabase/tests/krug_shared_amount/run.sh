...
# Pinned to the delivery this package guards; later Krug migrations have their own packages.
MIG="${KSA_MIG:-$(grep -l 'shared_amount_exceeds_amount' "$ROOT"/drizzle/migrations/*.sql | xargs grep -L 'krug_override_party' | sort | tail -1)}"
