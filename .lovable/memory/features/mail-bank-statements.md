...
- **Mrtvi „povezan" dokumenti (rujan 2026)**: status `povezan` smije nastati SAMO uz postojeći redak u `imported_statements` — klijent piše kroz RPC `mail_item_mark_linked` (izravni `update` je uklonjen). `undo_import_batch` u ISTOJ transakciji briše mrtvu `document_links` vezu i vraća dokument u `na_pregledu`. Oslobađanje zatečenih slučajeva: RPC `mail_item_release_linked` (odbija uz `uvoz_postoji`) + popis `mail_items_stuck_linked`, prikazan u Dokumenti → Primljeno („Označeno obrađenim bez uvoza"). Čuvari: `src/test/mailStuckLinkedRelease.test.ts`, `src/test/documentsStuckLinkedSection.test.tsx`.
